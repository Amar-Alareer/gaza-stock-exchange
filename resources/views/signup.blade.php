  ...
  <div class="auth-card row g-0 w-100 overflow-hidden shadow-lg rounded-5" style="max-width: 1000px;">
    ...
    <div class="col-12 col-md-6 auth-panel-left bg-light p-4 p-lg-5">
      ...
      <div class="d-flex p-1 bg-secondary bg-opacity-25 rounded-pill mb-4 mx-auto" style="max-width: 320px;">
        <a href="{{route('login')}}" class="btn flex-fill rounded-pill fw-bold text-muted">تسجيل دخول</a>
        <a href="{{route('signup')}}" class="btn flex-fill rounded-pill fw-bold active-tab-btn">إنشاء حساب</a>
      </div>
      ...
      <form action="{{route('signup.post')}}" method="POST">
        ...
        <div class="mb-2">
          ...
          <input type="text" name="name" class="form-control border-0 bg-white" placeholder="مثال: أحمد محمد" value="{{ old('name') }}" required>
        </div>

        <div class="mb-2">
          <label class="form-label fw-bold small mb-1">البريد الإلكتروني *</label>
          <input type="email" name="email" class="form-control border-0 bg-white" placeholder="example@gmail.com" value="{{ old('email') }}" required>
        </div>

        <div class="mb-2">
          <label class="form-label fw-bold small mb-1">رقم الهاتف</label>
          <input type="tel" name="phone" class="form-control border-0 bg-white" placeholder="0590000000" value="{{ old('phone') }}">
        </div>

        <div class="mb-2">
          <label class="form-label fw-bold small mb-1">المنطقة / الحي السكني</label>
          <select name="region_id" class="form-select border-0 bg-white">
            <option value="">اختر منطقتك السكنية (اختياري)</option>
            @if(isset($regions))
        ...
        <div class="mb-3">
          ...
          <input type="password" name="password" class="form-control border-0 bg-white" placeholder="••••••••••••" required>
        </div>

        <button type="submit" class="btn-locate w-100 justify-content-center py-2.5 fs-6 mb-3 d-flex align-items-center gap-2">
      ...
      <a href="{{ route('auth.google') }}" class="btn btn-outline-secondary w-100 py-2 rounded-3 fw-bold d-flex align-items-center justify-content-center gap-2 bg-white text-dark text-decoration-none shadow-sm">
